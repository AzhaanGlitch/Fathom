        ...
        <div className="max-w-5xl mx-auto space-y-8 pb-12">
            ...
            <div className="grid grid-cols-3 gap-4">
                <div className="bg-white/5 border border-white/10 rounded-xl p-5">
                    ...
                    <span className="text-3xl font-bold text-white">{loading ? '—' : mentors.length}</span>
                </div>
                <div className={`border rounded-xl p-5 ${duplicateGroups.length > 0 ? 'bg-red-500/10 border-red-500/20' : 'bg-white/5 border-white/10'}`}>
                    <span className="text-xs text-gray-500 uppercase tracking-wider block mb-1">Duplicate Groups</span>
                    <span className={`text-3xl font-bold ${duplicateGroups.length > 0 ? 'text-red-400' : 'text-white'}`}>
                        {loading ? '—' : duplicateGroups.length}
                    </span>
                </div>
                <div className={`border rounded-xl p-5 ${totalDuplicates > 0 ? 'bg-red-500/10 border-red-500/20' : 'bg-white/5 border-white/10'}`}>
                    ...
                    <div className="flex items-start justify-between mb-4">
                        <div>
                            ...
                            <p className="text-sm text-gray-400 max-w-xl">
                                Finds all mentor records that share the same <code className="text-orange-300 bg-white/5 px-1 rounded text-xs">owner_uid</code> and
            ...
            {duplicateGroups.length > 0 && (
                <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
                    ...
                    >
                        ...
                        {showMentorList ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
                    </button>
                            ...
                            {duplicateGroups.map(([ownerUid, list]) => (
                                <div key={ownerUid} className="px-6 py-4">
                                    <p className="text-xs text-gray-500 font-mono mb-3">
                                        owner_uid: <span className="text-gray-300">{ownerUid}</span>
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {mentors.map(m => {
                                        ...
                                        return (
                                            <tr key={m.id} className={`group hover:bg-white/5 transition-colors ${isDupe ? 'bg-red-500/5' : ''}`}>
                                                ...
                                                <td className="px-5 py-3 text-white">{m.total_sessions || 0}</td>
                                                <td className="px-5 py-3 text-white">{m.average_score ? m.average_score.toFixed(1) : '—'}</td>
            ...
            {/* Info box */}
            <div className="p-5 bg-white/5 border border-white/5 rounded-2xl text-xs text-gray-500 leading-relaxed space-y-1">
                <p><strong className="text-gray-300">Why do duplicates happen?</strong> Each call to <code className="text-orange-300">POST /api/mentors</code> previously always inserted a new record. After the fix, the backend checks for an existing <code className="text-orange-300">owner_uid</code> first and returns the existing record instead of creating a new one.</p>
                <p><strong className="text-gray-300">What does Merge do?</strong> For each group of duplicates it keeps the record with the most sessions (the "real" one), re-points all <code className="text-orange-300">sessions.mentor_id</code> foreign keys to the winner, then deletes the empty duplicates.</p>
            </div>
        </div>
    );
};
